...
public class KernelTools {
	...
	public static <K extends RealType<K>> Img<K> adjustKernelDimensions(
		...
		return new ImgView<K>(Views.interval(res, new FinalInterval(resDims)),
				kernel.factory());
	}
	...
	public static <K extends RealType<K> & NativeType<K>> Img<K>[] decomposeKernel(
		...
		SingularValueDecomposition svd = isDecomposable(SubsetViews.subsetView(
				kernel, kernel));
		...
		if (svd != null) {
			...
			decomposed[0] = KernelTools.adjustKernelDimensions(
					kernel.numDimensions(), new int[] { kernelDims[0] },
					vectorToImage(v, type, 1, new ArrayImgFactory<K>()));
			decomposed[1] = KernelTools.adjustKernelDimensions(
					kernel.numDimensions(), new int[] { kernelDims[1] },
					vectorToImage(u, type, 1, new ArrayImgFactory<K>()));
			return decomposed;
	...
	public static <R extends RealType<R>> Img<R> vectorToImage(
			final RealVector ar, final R type, int numDims, ImgFactory<R> fac) {
	...
	public static <R extends RealType<R>> RealMatrix toMatrix(final Img<R> img) {
		...
		while (c.hasNext()) {
			c.fwd();
			ret.setEntry(c.getIntPosition(0), c.getIntPosition(1), c.get()
					.getRealDouble());
		}
		return ret;
	}

	private static class ImgBasedRealMatrix<TT extends RealType<TT>> extends
			AbstractRealMatrix {
