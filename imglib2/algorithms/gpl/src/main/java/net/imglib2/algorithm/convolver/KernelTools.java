...
public class KernelTools {
	...
	public static <K extends RealType<K> & NativeType<K>> Img<K>[] decomposeKernel(
		...
		if (svd != null) {
			...
			decomposed[0] = KernelTools.adjustKernelDimensions(kernel
					.numDimensions(), new int[] { kernelDims[0] },
					ApacheMathTools.vectorToImage(v, type, 1,
							new ArrayImgFactory<K>()));
			decomposed[1] = KernelTools.adjustKernelDimensions(kernel
					.numDimensions(), new int[] { kernelDims[1] },
					ApacheMathTools.vectorToImage(u, type, 1,
							new ArrayImgFactory<K>()));
			return decomposed;
