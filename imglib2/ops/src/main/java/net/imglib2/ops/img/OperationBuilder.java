...
public class OperationBuilder
{
	...
	public static < A > UnaryOperation< A, A > concat( final BufferFactory< A > buff, final UnaryOperation< A, A >... ops )
	{
		return new ConcatenatedUnaryOperation< A >( buff, ops );
	}
	...
	public static < A, B, C > UnaryOperation< A, C > concat( final BufferFactory< B > buffer, final UnaryOperation< A, B > first, final UnaryOperation< B, C > second )
	{
		...
		// SingletonBufferFactory< B > singletonFactory = new
		// SingletonBufferFactory< B >( buffer );
		//
		// if ( first instanceof BufferedOperation && ( ( BufferedOperation< B >
		// ) first ).bufferFactory().equals( buffer ) )
		// ( ( BufferedOperation< B > ) second ).setBufferFactory(
		// singletonFactory );
		//
		// if ( second instanceof BufferedOperation && ( ( BufferedOperation< B
		// > ) first ).bufferFactory().equals( buffer ) )
		// ( ( BufferedOperation< B > ) second ).setBufferFactory(
		// singletonFactory );
