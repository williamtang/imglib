...
public class OperationBuilderTest {
	...
	public void init() {
		...
		buffer = new ArrayImgFactory<DoubleType>().create(dims,
				new DoubleType());
	...
	public void simpleMapTest() {
		...
		OperationBuilder.map(
				OperationBuilder.concat(new DoubleType(),
						new RealAdd<DoubleType, DoubleType, DoubleType>(),
						new RealMultiplyConstant<DoubleType, DoubleType>(
								constant))).compute(inA, inB, out);

		// Equation would be (valA+valB)*constant
		assertTrue(out.cursor().next().getRealDouble() == (valA + valB)
				* constant);
	...
	public void bufferedImgConcatEvenTest() {
		...
		for (int i = 0; i < ops.length; i++) {
			ops[i] = OperationBuilder
					.map(new RealAddConstant<DoubleType, DoubleType>(constant));
		}

		OperationBuilder.concat(
				buffer,
				OperationBuilder
						.map(new RealSubtractConstant<DoubleType, DoubleType>(
								constant)), ops).compute(inA, out);

		// Equation would be (valA+valB)*constant
		assertTrue(out.cursor().next().getRealDouble() == (valA - constant)
				+ (even * constant));
	}
	...
	public void bufferedTypeConcatEvenTest() {
		...
		DoubleType out = OperationBuilder.concat(new DoubleType(), mean, ops)
				.compute(inA.iterator(), new DoubleType());
